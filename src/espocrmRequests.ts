...
export const createEspoContact = async (contactData: EspoCRMContact) => {
    if (!espocrm_url || !espocrm_api_key) throw new Error('EspoCRM configuration missing');
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error("EspoCRM fetch failed: ", response.status, response.statusText, errorText);
            throw new Error(`EspoCRM response failed with: ${response.statusText} ${response.status}`);
        }
